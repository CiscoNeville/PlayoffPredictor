...
#get current ratings for all teams into hash by doing Matrix over scoreFile played to that point (at least historical to start)
...
$o = (($colNum - 1) * 379) + 16;
$resultsWon[$r] = "$teamRk[$q]:<div id=\"inner\" style=\"position: absolute; top:$p"."px; left:$o"."px; width:350px; height:18px; background-color:#008000\"> <font color=\"#FFF0FF\">W-($hTotal-$aTotal) - $aTeamName  (Rank #$teamRk[$q])</font></div>";
$wins = $wins + 1;
$r++;
}
else  {    #my team lost...
...
$p = ($teamRk[$q] * 8) + 290;
$o = (($colNum - 1) * 379) + 16;
$resultsLost[$s] = "$teamRk[$q]:<DIV id=\"inner\" style=\"position: absolute; top:$p"."px; left:$o"."px; width:350px; height:18px; background-color:#A00000\"> <font color=\"#F0FFFF\">L-($hTotal-$aTotal) - $aTeamName  (Rank #$teamRk[$q])</font></div>";
$losses = $losses + 1;
$s++;
}
}
...
#Add this result to my team's schedule/results array
 ...
 $p = ($teamRk[$q] * 8) + 290;
 $o = (($colNum - 1) * 379) + 16;
 $resultsWon[$r] = "$teamRk[$q]:<div id=\"inner\" style=\"position: absolute; top:$p"."px; left:$o"."px; width:350px; height:18px; background-color:#008000\"> <font color=\"#FFF0FF\">W-($aTotal-$hTotal) - $hTeamName  (Rank #$teamRk[$q])</font></div>";
$wins = $wins + 1;
$r++;
}
else {    #my team lost...
...
$p = ($teamRk[$q] * 8) + 290;
$o = (($colNum - 1) * 379) + 16;
 $resultsLost[$s] = "$teamRk[$q]:<DIV id=\"inner\" style=\"position: absolute; top:$p"."px; left:$o"."px; width:350px; height:18px; background-color:#A00000\"> <font color=\"#F0FFFF\">L-($aTotal-$hTotal) - $hTeamName  (Rank #$teamRk[$q])</font></div>";
$losses = $losses + 1;
$s++;
}
}
...
echo "<td>";			
echo "<table bgcolor =\"#FFFFFF\" border=\"0px\" cellpadding=\"\" cellspacing=\"\" width=\"359px\" height=\"1070px\"  frame=\"box\">";
...
echo "For year $year week $week<br>";
...
echo "<tr><th><DIV id=\"inner\" style=\"position: absolute; top:520px; left:725px; width:400px; height:20px;\">    <form method=\"get\" action=\"analyzeSchedule.php\"> <input type=\"hidden\" name=\"team1\" value=\"$team1In\">  <input type=\"hidden\" name=\"team2\" value=\"$team2In\"> <input type=\"text\" name=\"team3\" value=\"\"> <input type=\"submit\" value=\"Compare!\">      </div>   </th></tr>";	
print "<tr><th><DIV id=\"inner\" style=\"position: absolute; top:560px; left:780px; width:350px; height:20px;\"> to compare their schedules to side-by-side  </div>   </th></tr>";	
}	
}	
